# Social Login with Amazon Cognito (Google)

> AI-generated reference implementation. Review carefully before production use.

Federated social login via Google, using Amazon Cognito Managed Login v2 and the OAuth 2.0 Authorization Code Flow with PKCE. Users never create a Cognito-native password — authentication is fully delegated to Google.
...
The Google setup step requires your Cognito domain, which is created by CloudFormation. The domain is predictable — it will be `https://<stack-name>.auth.<region>.amazoncognito.com` — but Cognito domain prefixes are **globally unique**, so the name you choose might already be taken.

To avoid configuring Google and then hitting a name collision during deployment, deploy with a placeholder Google client secret first to claim the domain:
...
Once the stack is created successfully, your domain is confirmed and reserved. Then complete the Google setup below and update the stack with real credentials:
...
Your confirmed Cognito domain (substitute into the step below):
...
Follow the two-stage process described in [Before you start](#before-you-start-claim-your-cognito-domain): deploy with placeholders first to claim the domain, complete the Google setup, then redeploy with real credentials.
